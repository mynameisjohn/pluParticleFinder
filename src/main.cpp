...
// parse XML params and execute
int parseXMLAndExecute (pugi::xml_document paramDoc)
{
    ParticleFinder P;
...
DLL_EXPORT int runFromUnity (const char* xmlString)
{
    pugi::xml_document paramDoc;
    if (!paramDoc.load_string (xmlString))
        return EXIT_FAILURE;

    return parseXMLAndExecute (std::move (paramDoc));
}
...
int main( int argc, char ** argv )
{
    ...
    // This just runs using the default centerfind args
    ParticleFinder P;
    std::list<std::string> inputFiles;
    int startOfStack{ 0 }, endOfStack{ 0 };
    bool getUserInput = false;
